...
class SetUpFreeTimeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        let reminderArray = checkList[checkList[checkList.index(checkList.startIndex, offsetBy: indexPath.section)].key]
        var reminder = Reminder()
        reminder = (reminderArray?[indexPath.row])!
        checkList[checkList[checkList.index(checkList.startIndex, offsetBy: indexPath.section)].key]?.remove(at: indexPath.row)
